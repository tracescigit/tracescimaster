    ...
    public function store(SupplyChainUserCreateRequest $request)
    {
        try {
            ...
            if ($request->hasFile('user_image')) {
                ...
                $fileName = 'user_' . Str::uuid() . '.' . $image->getClientOriginalExtension();


                $image->storeAs('images', $fileName, 'public');

                $user->user_image = 'images/' . $fileName;
            }
    ...
    public function update(SupplyChainUserUpdateRequest $request, $id)
    {
        try {
            ...
            if ($user) {
                ...
                if ($request->hasFile('user_image')) {
                    // Delete old image
                    if (!empty($user->user_image) && Storage::disk('public')->exists($user->user_image)) {
