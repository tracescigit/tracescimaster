...
class ReportController extends Controller
{
    ...
    {   
        ...
        {   
            ...
            $response       = Alert::getReportModel($limit, $page, $orderby, $order, $search_field , $search_type, $search_value,null,$start_date,$end_date);
            ...
            foreach ($reports as $report) {
                ...
                $u['code_data']             = $report->getCode->code_data??($report->product_name??'-');
                $u['user_name']             = $report->getUser?($report->getUser->first_name.' '.$report->getUser->last_name??'-'):'-';
                $u['created_at']            = date('M d, Y',strtotime($report->created_at))??'-';
                $actions                    = view('admin.reports.actions',['id' => $report->id,'admin_assigned_to'=>$report->admin_assigned_to]);
    ...
    {   
        ...
        return view('admin.reports.details')->with('report',$report)->with('page_name','admin-reports');
    }
    ...
    {   
        ...
        $report->admin_assigned_to = $input['assigned_to'];
        $report->save();
    }
