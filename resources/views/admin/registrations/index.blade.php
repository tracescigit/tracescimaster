...
@section('subhead')
<title>Registrations - {{ env('APP_NAME') }}</title>
@endsection

@section('subcontent')
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
	<h2 class="text-lg font-medium mr-auto">Registrations</h2>
	@if (hasRoutePermission('admin-create-registrations',Auth::id()))
	<div class="w-full sm:w-auto flex mt-4 sm:mt-0">
		<a href="{{ route('admin-create-registrations') }}" class="btn btn-primary shadow-md mr-2">{{__('registration.new_registration')}}</a>
		<div class="dropdown ml-auto sm:ml-0">
			<a href="{{ route('admin-create-registrations') }}" class="btn px-2 box text-gray-700 dark:text-gray-300" aria-expanded="false">
		...
		<form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto col-span-12" >
			...
			<div class="sm:flex items-center sm:mr-4">
				...
				<select id="tabulator-html-filter-field" class="form-select w-full sm:w-32 xxl:w-full mt-2 sm:mt-0 sm:w-auto">	
					...
					{{-- <option value="city">{{__('common.city')}}</option> --}}
					{{-- <option value="country">{{__('common.country')}}</option> --}}
					<option value="status">{{__('common.status')}}</option>
					<option value="active">{{__('common.document_verified')}}</option>
				</select>
			</div>
