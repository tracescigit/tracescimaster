...
@section('subhead')
<title>Update Topups - TRACESCI</title>
@endsection

@section('subcontent')
<div class="intro-y flex items-center mt-8">
	<h2 class="text-lg font-medium mr-auto">Update Topup</h2>
</div>
					...
					<div class="grid grid-cols-12">
						...
						<div class="input-form col-span-12 lg:col-span-6 px-2 py-1">
							...
							<select id="parent_plan" name="parent_plan" class="form-select form__input">
								...
								<option value="{{$plan->id}}" {{$plan->id==$topup->parent_id?'selected':''}}>{{$plan->title}}</option>
								@endforeach
							</select>
						</div>
						...
						<div class="input-form col-span-12 lg:col-span-6 px-2 py-1">
							...
							<input id="title" type="text" name="title" class="form-control form__input" value="{{$topup->title}}" placeholder="Enter title" minlength="2">
						...
						<div class="input-form col-span-12 lg:col-span-6 px-2 py-1 mt-2">
							...
							<input id="credits" type="number" name="credits" class="form-control form__input" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" min="1" step="1" maxlength="10" value="{{$topup->credits}}" placeholder="Enter credits" minlength="1">
						...
						<div class="input-form col-span-12 lg:col-span-6 px-2 py-1 mt-2">
							...
							<input id="price_inr" type="number" name="price_inr" class="form-control form__input" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" min="0" step="0.01" maxlength="10" value="{{number_format((float)$topup->price_inr,2,'.','')}}" placeholder="Enter price in inr" minlength="2">
						...
						<div class="input-form col-span-12 lg:col-span-6 px-2 py-1 mt-2">
							...
							<input id="price_usd" type="number" name="price_usd" class="form-control form__input" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" min="0" step="0.01" maxlength="10" value="{{number_format((float)$topup->price_usd,2,'.','')}}" placeholder="Enter price in usd" minlength="2">
						...
						<div class="input-form col-span-12 lg:col-span-6 px-2 py-1 mt-2">
							...
							<select id="status" name="status" class="form-select form__input">
								...
								<option value="0" {{$topup->status=='0'?'selected':''}}>Inactive</option>
							</select>
						</div>
						...
						<div class="input-form col-span-12 lg:col-span-12 px-2 py-1 mt-2">
							...
							<textarea id="description" rows="5" name="description" class="form-control form__input tinymce" placeholder="Enter description" minlength="2">{{$topup->description}}</textarea>
						...
						<div class="input-form col-span-12 lg:col-span-12 px-2 py-1 mt-3">
							<button type="submit" id="btn-update" class="btn btn-primary w-full xl:w-32 xl:mr-3 align-top">Update topup</button>
						</div>
					</div>
				</div>
			</div>
	...
	cash(function () {
		async function add() {
			...
			axios.post('{{ url('/admin/topups/'.encrypt($topup->id).'/edit') }}', formData).then(res => {
				...
				cash('#btn-update').html('Update topup')                   
