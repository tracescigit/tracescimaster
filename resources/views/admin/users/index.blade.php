...
@section('subhead')
<title>{{__('common.users')}} TRACESCI</title>
@endsection
@section('subcontent')

<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
	<h2 class="text-lg font-medium mr-auto">{{__('common.users')}}</h2>
	@if (hasRoutePermission('admin-create-users',Auth::id()))
	<div class="w-full sm:w-auto flex mt-4 sm:mt-0">
		<a href="{{ route('admin-create-users') }}" class="btn btn-primary shadow-md mr-2">{{__('common.add_user')}}</a>
		<div class="dropdown ml-auto sm:ml-0">
			<a href="{{ route('admin-create-users') }}" class="btn px-2 box text-gray-700 dark:text-gray-300" aria-expanded="false">
...
<script>
	var tabulatorUrl =  '{{ route('admin-users') }}';
