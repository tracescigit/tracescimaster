...
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
	...
	{{-- <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
		<a href="{{ route('admin-create-products') }}" class="btn btn-primary shadow-md mr-2">Add New Product</a>
		<div class="dropdown ml-auto sm:ml-0">
			<a href="{{ route('admin-create-products') }}" class="btn px-2 box text-gray-700 dark:text-gray-300" aria-expanded="false">
	...
	<div class="flex flex-col sm:flex-row sm:items-end xl:items-start">
		<form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto" >
			<div class="sm:flex items-center sm:mr-4">
				...
				<select id="tabulator-html-filter-field" class="form-select w-full sm:w-32 xxl:w-full mt-2 sm:mt-0 sm:w-auto">
					...
					<option value="companies.name">Business Name</option>
					<option value="users.name">User Name</option>
					<option value="price">Price</option>
					<option value="brand">Brand</option>
					<option value="products.status">Status</option>
				</select>
			</div>
		...
		<script>
			...
			},
			{
				...
				download: false,
				formatter: function formatter(cell, formatterParams) {
					return '<div><div class="font-medium whitespace-nowrap">'+ cell.getData().user_id +'<br>'+cell.getData().email+' </div>';
				}
			},
			{
				title: "PRICE",
			...
			}, 
			{
				...
				download: false,
				formatter: function formatter(cell, formatterParams) {
					return "<div class=\"flex items-center lg:justify-center ".concat(cell.getData().product_status=='1' ? "text-theme-9" : "text-theme-6", "\">\n                            <i data-feather=\"check-square\" class=\"w-4 h-4 mr-2\"></i> ").concat(cell.getData().product_status=='1' ? "Active" : "Inactive", "\n                        </div>");
				}
			},
