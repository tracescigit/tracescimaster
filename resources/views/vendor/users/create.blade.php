...
@section('subhead')
<title>{{__('user.create_users')}}</title>
@endsection
								...
								<div class="input-form col-span-9 px-2 py-1">
									<div class="flex flex-col sm:flex-row">
										<div class="form-check mr-5">
											<input id="{{$module->slug}}-view" class="form-check-input checkboxes" type="checkbox" name="view[]" value="{{$module->id}}" {{$module->slug=='dashboard'?'checked':''}}>
											<label class="form-check-label" for="{{$module->slug}}-view">{{__('common.view')}}</label>
										</div>

										@if ($module->slug!='dashboard')
										<div class="form-check mr-2 mt-2 sm:mt-0">
											<input id="{{$module->slug}}-modify" class="form-check-input checkboxes" type="checkbox" name="modify[]" value="{{$module->id}}">
	...
	cash(function () {
		...
		async function brandAction(){
			...
			if(value=='Brand User'){
				...
				cash('#dashboard-view').prop('checked', true);
				cash('#brand').hide()
			}
		}
	})
