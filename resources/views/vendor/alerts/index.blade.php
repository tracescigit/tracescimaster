...
@section('subhead')
<title>Alerts - TRACESCI</title>
@endsection
@section('subcontent')
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
	<h2 class="text-lg font-medium mr-auto">Alerts</h2>
</div>
		...
		<div class="report-box zoom-in">
			<div class="box p-5">
				...
				<div class="text-3xl font-bold leading-8 mt-6">
					{{getTotalAlertsVendor(Auth::user()->parent_id??Auth::id())}}
				</div>
				<div class="text-base text-gray-600 mt-1">Total Alerts</div>
			</div>
		</div>
	</div>
	<div class="col-span-12 lg:col-span-3 intro-y">
		<div class="report-box zoom-in">
			<div class="box p-5">
				...
				<div class="text-3xl font-bold leading-8 mt-6">{{getActiveAlertsVendor(Auth::user()->parent_id??Auth::id())}}</div>
				<div class="text-base text-gray-600 mt-1">To Act</div>
			</div>
		</div>
	</div>
</div>
	...
	<div class="pos__ticket mt-5 p-5">
		<div class="col-span-12 lg:col-span-12 bg-gray-200 p-3 box">
			...
			<p class="mb-2 text-md">
				This is the page where the system will give you alerts on possible fraudulent activities or counterfeit products. To see more details and take further action on any alert, click on ‘Act now’. To initiate investigation on the alert, assign the case to your operations team. This person will upload the alert report on the portal, which will be visible to the management and other relevant authorities.
			</p>
		</div>
	</div>
</div>
		...
		<form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto col-span-12" >
			...
			<div class="sm:flex items-center sm:mr-4">
				...
				<select id="tabulator-html-filter-field" class="form-select w-full sm:w-32 xxl:w-full mt-2 sm:mt-0 sm:w-auto">
					...
					<option value="alert_message">{{__('alert.alert_message')}}</option>
					<option value="admin_assigned_to">{{__('alert.action_taken')}}</option>		
				</select>
			</div>
...
<script>
	var tabulatorUrl =  '{{ route('vendor-alerts') }}';
