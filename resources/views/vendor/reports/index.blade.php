...
@section('subhead')
<title>App Reports - TRACESCI</title>
@endsection
@section('subcontent')
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
	<h2 class="text-lg font-medium mr-auto">App Reports</h2>
</div>
		...
		<form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto col-span-12" >
			...
			<div class="sm:flex items-center sm:mr-4">
				...
				<select id="tabulator-html-filter-field" class="form-select w-full sm:w-32 xxl:w-full mt-2 sm:mt-0 sm:w-auto">
					...
					<option value="products.name">{{__('common.product_name')}}</option>
					<option value="code_data">Product Serial No.</option>
					<option value="users.name">{{__('report.created_by')}}</option>
					<option value="batch">{{__('common.batch_code')}}</option>
					<option value="issue_type">{{__('report.issue_type')}}</option>				
				</select>
			</div>
...
<script>
	var tabulatorUrl =  '{{ route('vendor-reports') }}';
	...
	{	
		...
		title: "{{strtoupper(__('report.issue_type'))}}",
		minWidth: 200,
		responsive: 0,
		field: "issue_type",
		hozAlign: "center",
		...
		title: "{{strtoupper(__('report.reported_by'))}}",
		minWidth: 200,
		field: "user_name",
		hozAlign: "center",
		vertAlign: "middle",
